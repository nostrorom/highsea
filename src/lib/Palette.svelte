			...
			{#each color.levels as level}
				<div class="font-mono">
					<div class="h-12 w-full rounded-lg relative" style="background:{level['hex']}">
						<div
							class="bg-white dark:bg-black text-slate-500 dark:text-slate-400 rounded-br-md px-1 absolute top-0 left-0 xl:hidden"
						>
							{level.id}
						</div>
					</div>
					<div class="flex justify-between px-1 pt-0.5 space-x-1 font-mono">
						<p class="text-slate-400 dark:text-slate-400 hidden xl:block">
							{level.id}
						</p>
